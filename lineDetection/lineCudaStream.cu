...
int main(int argc, char **argv)
{
  ...
  cudaMalloc(&line_d, nb_pixels + cols);
  ...
  line<<<b, t, 0, streams[0]>>>(line_d, g_d, cols, (rows / 2) + 1);
  line<<<b, t, 0, streams[1]>>>(line_d + nb_pixels / 2, g_d + nb_pixels / 2 - cols, cols, (rows / 2) - 1);

  cudaEventRecord(stop, streams[0]);

  // copie de l'image du GPU vers l'hôte
  cudaMemcpyAsync(res, line_d, nb_pixels / 2, cudaMemcpyDeviceToHost, streams[0]);
  cudaMemcpyAsync(res + nb_pixels / 2, line_d + nb_pixels / 2 + cols, nb_pixels / 2, cudaMemcpyDeviceToHost, streams[1]);
