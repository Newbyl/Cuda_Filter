...
int main(int argc, char **argv)
{
    ...
    for (j = 1; j < height - 1; ++j)
    {
        for (i = 1; i < width - 1; ++i)
        {
            ...
            res = -1 * pixels[((j - 1) * width + i - 1)] + -1 * pixels[((j - 0) * width + i - 1)] + 2 * pixels[((j + 1) * width + i - 1)] +
                  -1 * pixels[((j - 1) * width + i - 0)] + 2 * pixels[((j - 0) * width + i + 0)] + -1 * pixels[((j + 1) * width + i + 0)] +
                  2 * pixels[((j - 1) * width + i + 1)] + -1 * pixels[((j - 0) * width + i + 1)] + -1 * pixels[((j + 1) * width + i + 1)];
