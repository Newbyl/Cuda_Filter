...
// unsigned char *laplacian = matrice de sortie, g = matrice d'entree
__global__ void laplacian(unsigned char *laplacian, unsigned char *g, std::size_t cols, std::size_t rows)
{
  ...
  if (i < cols + 1 && j < rows + 1 && i > 0 && j > 0)
  {
    ...
    auto res = 0 * g[((j - 1) * cols + i - 1)] + -1 * g[((j - 0) * cols + i - 1)] + 0 * g[((j + 1) * cols + i - 1)] +
               -1 * g[((j - 1) * cols + i - 0)] + 4 * g[((j - 0) * cols + i + 0)] + -1 * g[((j + 1) * cols + i + 0)] +
               0 * g[((j - 1) * cols + i + 1)] + -1 * g[((j - 0) * cols + i + 1)] + 0 * g[((j + 1) * cols + i + 1)];

    // on normalise le pixel (pas d'overshoot / undershoot )
    res = res > 255 ? res = 255 : res;
    res = res < 0 ? res = 0 : res;

    laplacian[j * cols + i] = res;
  }
}
...
int main(int argc, char **argv)
{
  ...
  cudaMalloc(&laplacian_d, nb_pixels + cols);
  ...
  laplacian<<<b, t, 0, streams[0]>>>(laplacian_d, g_d, cols, (rows / 2) + 1);
  laplacian<<<b, t, 0, streams[1]>>>(laplacian_d + nb_pixels / 2, g_d + nb_pixels / 2 - cols, cols, (rows / 2) - 1);

  cudaEventRecord(stop, streams[0]);

  // copie de l'image du GPU vers l'hôte
  cudaMemcpyAsync(res, laplacian_d, nb_pixels / 2, cudaMemcpyDeviceToHost, streams[0]);
  cudaMemcpyAsync(res + nb_pixels / 2, laplacian_d + nb_pixels / 2 + cols, nb_pixels / 2, cudaMemcpyDeviceToHost, streams[1]);
