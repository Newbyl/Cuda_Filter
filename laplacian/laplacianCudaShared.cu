...
__global__ void laplacian(unsigned char *laplacian, unsigned char *g, std::size_t cols, std::size_t rows)
{
    ...
    */
    ...
    {
        int res = 0 * shared_g[(shared_j - 1) * w + shared_i - 1] + -1 * shared_g[shared_j * w + shared_i - 1] + 0 * shared_g[(shared_j + 1) * w + shared_i - 1] +
                  -1 * shared_g[(shared_j - 1) * w + shared_i - 0] + 4 * shared_g[shared_j * w + shared_i + 0] + -1 * shared_g[(shared_j + 1) * w + shared_i + 0] +
                  0 * shared_g[(shared_j - 1) * w + shared_i + 1] + -1 * shared_g[shared_j * w + shared_i + 1] + 0 * shared_g[(shared_j + 1) * w + shared_i + 1];
...
int main(int argc, char **argv)
{
    ...
    cudaMalloc(&laplacian_d, rows * cols);
    ...
    laplacian<<<grid, block, block.x * block.y>>>(laplacian_d, g_d, cols, rows);

    cudaEventRecord(stop);

    cudaMemcpy(g.data(), laplacian_d, rows * cols, cudaMemcpyDeviceToHost);

    cudaEventSynchronize(stop);

    float ms = 0;
    cudaEventElapsedTime(&ms, start, stop);
    cv::imwrite("outshare.jpg", m_out);
